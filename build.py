import os
from beet import Context, subproject, ListOption
...
class SmithedOptions(BaseModel):
    libraries: ListOption[str] = []
    zip: bool = True
    release: bool = False
...
def beet_default(ctx: Context):
    ...
    filtered = filter(
        lambda item: ((item.name in libraries))
        if libraries
        else (item.name != "template"),
        items,
    )
    ...
    for file in filtered:
        if file.is_dir():
            try:
                ...
                if release:
                    config = yaml.load(
                        open(str(file) + "/beet.yaml", "r"), Loader=yaml.FullLoader
                    )
                    ...
                    subproject(
                        {
                            ...
                            "meta": {
                                "render": {"data_pack": {"functions": ["smithed.*"]}}
                            },
                        }
                    )
                )
