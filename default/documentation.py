...
def parse_doc(line: str, search: Match[str], parent_folder: str):
    ...
    if not os.path.isdir(containing_folder): os.makedirs(containing_folder)
    return path
...
Input = namedtuple('Input', ['name','type','source','target'], defaults=[None, None, None, None])
def collect_doc(path: str, lines: List[str], i: int):
    doc = ''
    inputs = [Input('Input Name', 'Input Type', 'Input Source', 'Input Objective/Path'),Input('---','---','---','---')]
    ...
    while(lines[i].startswith('#')):
        ...
        if search := re.search(INPUT_REGEX, line):
            ...
            print(args)
            input = Input(f'"{args[0]}"', args[1].strip(), args[2].strip(), args[3].strip())
            inputs.append(input)
        else:
            doc += lines[i][re.search('^# ?', lines[i]).span(0)[1]:] + '\n'
        i += 1
    with open(path + '.md','w+') as doc_file:
        if len(inputs) > 2: doc_file.write(make_table(inputs))
        doc_file.write(doc)
    return i
