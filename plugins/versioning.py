...
def call_if_version_match(scoreholder, version_parts, path):
    return (
        "execute "
        + combine_parts(
            f"if score {scoreholder}.{{name}} load.status matches {{part}} ", version_parts
        )
        + f"run function {path}"
    )
...
def enumerate(ctx: Context, scoreholder, namespace, version):
    ...
    ctx.data[name] = Function(
        ...
        + combine_part_names(f"scoreboard players add {scoreholder}.{{name}} load.status 0\n")
        + f"function {enumerate_step_name(namespace, version_part_names[0], version)}"
    )
...
def set_version(ctx: Context, namespace, scoreholder, version, version_parts):
    ...
    ctx.data[name] = Function(
        combine_parts(
            f"scoreboard players set {scoreholder}.{{name}} load.status {{part}}", version_parts
        )
...
def generate_call(ctx, path: str, version: str, version_parts, namespace: str, scoreholder: str):
    ...
    ctx.data[call_path] = Function(call_if_version_match(scoreholder, version_parts, path))
...
def beet_default(ctx: Context):
    ...
    scoreholder = ctx.meta["versioning"]["scoreholder"]
    namespace = ctx.meta["versioning"]["namespace"]
    ...
    for (step, next_step), part in zip(step_pairs, version_parts):
        enumerate_step(ctx, scoreholder, namespace, version, step, next_step, part, set_version_func)
