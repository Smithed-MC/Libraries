...
raw execute unless data storage smithed.crafter:input {flags:["consume_tools"]} if items entity @s weapon.mainhand *[minecraft:max_damage,!minecraft:unbreakable,!minecraft:custom_data~{smithed:{not_tool:1b}}] run function smithed.crafter:impl/block/table/crafting/output/clear_input/handle_tools
